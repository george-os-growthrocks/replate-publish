...
const RequestSchema = z.object({
  ...
  depth: z.number().int().min(1).max(500).default(100),
  sort_by: z.enum(["most_relevant", "newest", "highest_rating", "lowest_rating"]).default("most_relevant"),
});
...
serve(async (req) => {
  ...
  try {
    ...
    const response = await client.post(
      "business_data/google/maps/reviews/live",
      {
