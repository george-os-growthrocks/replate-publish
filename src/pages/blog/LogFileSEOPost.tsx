  ...
  return (
    <>
      ...
      <BlogPostLayout
        ...
        description="Parse access logs, spot crawl traps, and fix budget-draining patterns with practical commands and queries."
        categoryBadges={["Technical SEO", "Advanced"]}
        date="October 28, 2025"
        readTime="9 min read"
        tocItems={[
          "Why Log-File Analysis Matters",
          "What to Look For in Your Logs",
          "The 5-Minute Crawl Audit",
      ...
      >
        ...
        <p>Parse your access.log for Googlebot user agents. Group by hour to spot patterns:</p>
        ...
        <p>Google doesn't tell you <em>all</em> the errors it encounters. Logs do:</p>
        ...
        <div className="not-prose my-6 space-y-3">
          {[
            { title: "Extract Googlebot Requests", desc: "Filter logs for Googlebot/Googlebot-Mobile user agents" },
            { title: "Count HTTP Status Codes", desc: "Group by 2xx, 3xx, 4xx, 5xx and calculate percentages" },
            { title: "Identify Top Crawled Pages", desc: "Are your most-crawled pages your most important? If not, investigate." },
            { title: "Check Crawl Timing", desc: "Group requests by hour—server slowness correlates with low crawl rates" },
            { title: "Compare Logs to Sitemap", desc: "Pages in sitemap but never crawled = potential indexation issues" },
          ].map((step, idx) => (
            <div key={idx} className="flex items-start gap-2 p-4 bg-muted/50 rounded-lg">
              <div className="w-6 h-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center flex-shrink-0 text-sm font-bold">{idx + 1}</div>
              <div>
        ...
        <div className="not-prose my-4 p-4 bg-slate-950 rounded-lg">
          <code className="text-green-400 text-sm font-mono">{`goaccess access.log -o report.html --log-format=COMBINED`}</code>
        </div>

        <h3>SQL for Deeper Analysis</h3>
        <p>Import logs into SQLite or PostgreSQL for flexible querying:</p>

        <div className="not-prose my-4 p-4 bg-slate-950 rounded-lg">
          <pre className="text-green-400 text-xs font-mono overflow-x-auto">{`-- Most crawled URLs with 404s
...
LIMIT 20;`}</pre>
        ...
        <p>For most sites, monthly is sufficient. E-commerce and news sites should review weekly during peak seasons or after major site changes.</p>
        <h3>Do I need expensive tools?</h3>
        <p>No. GoAccess, grep, and basic SQL cover 80% of use cases. Premium tools like Screaming Frog Log Analyzer add convenience but aren't mandatory.</p>
        <h3>What if I don't have log access?</h3>
        <p>Ask your host or dev team. Most managed hosting (Cloudflare, AWS, Netlify) provides log access. Without it, you're flying blind on technical SEO.</p>
